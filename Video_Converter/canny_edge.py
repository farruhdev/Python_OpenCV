...
def cannyDetect(img, i):
    sigma = 1.4
    kernelSize = 5
    imgSmoothed = convolve(img, gaussianKernel(kernelSize, sigma))
    gradientMat, thetaMat = sobelFilters(imgSmoothed)
    nonMaxImg = nonMaxSuppression(gradientMat, thetaMat)
    thresholdImg = threshold(nonMaxImg)
    imgFinal = hysteresis(thresholdImg)
